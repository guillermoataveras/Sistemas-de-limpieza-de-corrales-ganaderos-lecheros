...
// =====================================================
// PINES DIR AS5600
// El AS5600 lee el estado de DIR en el arranque.
// Deben configurarse ANTES de Wire.begin().
// LOW  = ángulo crece en sentido horario
// HIGH = ángulo crece en sentido antihorario
// =====================================================
static const uint8_t ENC_DIR_IZQ = 7;
static const uint8_t ENC_DIR_DER = 6;

// Cambia HIGH/LOW si la odometría muestra una rueda al revés
...
// Lectura periódica de encoders (ms)
// Separada del loop principal para no saturar el bus I2C
static const uint32_t ENC_READ_MS   = 20;  // 50 Hz

// Si los encoders fallan consecutivamente, se deshabilitan
// temporalmente para no bloquear el control del robot.
...
// Timeout del periférico I2C (us)
// Si el bus se cuelga, Wire aborta y libera el loop.
...
// Lectura periódica de gyro (opcionalmente desacoplada del loop)
static const uint32_t GYRO_READ_MS = 5; // 200 Hz aprox.
...
// Calcula el delta en grados manejando rollover 0↔4095
...
// =====================================================
// DIAGNÓSTICO DE ENCODERS AL ARRANQUE
// =====================================================
void diagEncoders() {
  ...
  if (tcaSelectSafe(TCA_CH_IZQ) && as5600ReadRawSafe(rawTest)) {
    Serial.print("INFO:AS5600 IZQ OK  raw=");
  ...
  if (tcaSelectSafe(TCA_CH_DER) && as5600ReadRawSafe(rawTest)) {
    Serial.print("INFO:AS5600 DER OK  raw=");
    Serial.println(rawTest);
  } else {
...
// No bloquea el loop: si fallan, se deshabilitan temporalmente.
// =====================================================
void updateEncoders() {
  ...
  if (tcaSelectSafe(TCA_CH_IZQ)) {
    okIzq = as5600ReadRawSafe(rawIzq);
  }

  if (tcaSelectSafe(TCA_CH_DER)) {
    okDer = as5600ReadRawSafe(rawDer);
  }

  tcaDisableAllSafe();

  // Primera lectura: guardar referencia, no acumular
  if (!encInitialized) {
    ...
    if (okIzq && okDer) {
      ...
      if (!okIzq) {
        ...
        Serial.println("INFO:Esperando AS5600 IZQ...");
      }
      if (!okDer) {
        if (encFailCountDer < 255) encFailCountDer++;
        Serial.println("INFO:Esperando AS5600 DER...");
      }
    }
    return;
  }
...
void stopBTS7960(uint8_t r_en, uint8_t l_en,
                 uint8_t rpwm, uint8_t lpwm) {
  digitalWrite(r_en, HIGH);
  digitalWrite(l_en, HIGH);
  analogWrite(rpwm, 0);
...
// =====================================================
// PARSER SERIAL
// Formato: CMD:izq,der,aux_pwm,comp
// Ej:      CMD:80,80,255,1
// =====================================================
...
// =====================================================
// SEGURIDAD — parada por timeout de comando
// =====================================================
...
void setup() {
  ...
  // Estado inicial BTS7960 (enables HIGH, PWM = 0)
...
// LOOP
// Orden de prioridad:
// 1) Serial y seguridad
// 2) Sensores I2C sin bloquear el control
// =====================================================
void loop() {
  handleSerial();

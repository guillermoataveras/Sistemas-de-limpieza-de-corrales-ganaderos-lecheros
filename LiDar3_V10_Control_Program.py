...
PATRON_SNAKE_DIAGONAL = "SNAKE-90"      # giros de 90° al cambiar de fila

PATRONES_CICLO   = [PATRON_NINGUNO, PATRON_MATRICIAL,
                    PATRON_ESPIRAL, PATRON_BOWTIE,
                    PATRON_SNAKE_DIAGONAL]

# Paso entre pasadas = 350mm (diámetro limpieza = 440mm → overlap ~90mm)
# El overlap asegura cobertura completa aunque el robot se desalinee
# ligeramente entre filas. El trade-off es ~20% más filas por recorrer.
PASO_LIMPIEZA_MM = 350   # mm  (antes 440 = sin solapamiento)
...
COLOR_PATRON = {
    ...
    PATRON_SNAKE_DIAGONAL: (100, 255, 180),   # verde-turquesa
}
...
def generate_snake_diagonal(bbox, paso_mm=None):
    """
    Snake-90: patrón de serpiente donde los cambios de fila se hacen con
    DOS giros de 90° en vez de un giro de 180°.

    Comparación con matricial boustrophedon clásico:
      - MATRICIAL: fila completa → giro 180° → fila completa inversa → ...
        Cada cambio = 1 giro de 180° difícil (robot debe rotar en sitio).
      - SNAKE-90:  fila completa → giro 90° (bajar) → paso lateral → giro
        90° (seguir) → fila completa inversa → ...
        Cada cambio = 2 giros de 90° cortos con una pequeña traslación entre ellos.

    Con el esquema de rotación-con-pausas del follower, los giros de 90°
    son mucho más rápidos y precisos que los giros de 180°. Aunque son
    más giros por cambio de fila, cada uno converge rápido.

    El camino genera waypoints así (para filas Y0, Y1, Y2...):
        (x_min, Y0) → (x_max, Y0)                         # fila 0 →
                   → (x_max, Y0 + paso/2)                 # baja medio paso
                   → (x_max, Y1) "implícito por cercanía" # llega a Y1
        (x_max, Y1) → (x_min, Y1)                         # fila 1 ←
                   → (x_min, Y1 + paso/2) → (x_min, Y2)   # bajada
        ...
    """
    if paso_mm is None:
        paso_mm = PASO_LIMPIEZA_MM
    ...
    while y <= y_max + 1:
        ...
        if row % 2 == 0:
            waypoints.append((x_min, y))
            waypoints.append((x_max, y))
            # Tras llegar al extremo derecho, bajar en L:
            # primero avanzar hacia abajo (manteniendo X), luego al siguiente Y
            y_next = y + paso_mm
            if y_next <= y_max + 1:
                waypoints.append((x_max, y_next))
        else:
            waypoints.append((x_max, y))
            waypoints.append((x_min, y))
            # En fila impar bajamos manteniendo X_min
            y_next = y + paso_mm
            if y_next <= y_max + 1:
                waypoints.append((x_min, y_next))
        y += paso_mm
        row += 1
    return waypoints
...
def generate_pattern(patron, grid_map):
    ...
    elif patron == PATRON_SNAKE_DIAGONAL:
        return generate_snake_diagonal(bbox)
    return []
...
def select_best_pattern(grid_map):
    ...
    # ── Decisión ─────────────────────────────────────────────────
    # Con la nueva FSM (rotación en sitio con pausas), los giros de 180°
    # son costosos: preferimos SNAKE_DIAGONAL (giros de 90°) sobre MATRICIAL.
    # MATRICIAL queda como opción manual (ciclable con tecla F2).

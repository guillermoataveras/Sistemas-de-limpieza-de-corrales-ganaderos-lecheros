...
def detect_wheel_slip(enc_delta_izq_deg, enc_delta_der_deg,
                       gyro_yaw_delta_deg):
    """Compatibilidad legado: en modo LiDAR-only no se usa."""
    return False, 0.0
...
class PoseKalmanFilter:
    """
    Filtro de pose simplificado para modo LiDAR-only.

    No usa una covarianza 2x2 completa. Solo mantiene una estimación
    de posición y una incertidumbre escalar aproximada para depuración.
    La corrección LiDAR se aplica por mezcla ponderada con alpha
    dependiente de la calidad del match.
    """

    def __init__(self, x0, y0, p_init=KALMAN_P_INIT_MM2):
        self.x = float(x0)
        self.y = float(y0)
        self.sigma2 = float(p_init)

    def predict(self, dx_mm, dy_mm, q_var_mm2=KALMAN_Q_BASE_MM2):
        self.x += dx_mm
        self.y += dy_mm
        self.sigma2 = min(self.sigma2 + float(q_var_mm2), 5000.0)
    ...
    def update_lidar(self, z_x, z_y, match_quality=None, r_var_mm2=None):
        if r_var_mm2 is None:
            if match_quality is None:
                ...
                q = clamp(match_quality, 0.0, 100.0)
                alpha = 0.15 + 0.60 * (q / 100.0)
        else:
            alpha = 0.15 if r_var_mm2 >= 150.0 else 0.45

        self.x = (1.0 - alpha) * self.x + alpha * float(z_x)
        self.y = (1.0 - alpha) * self.y + alpha * float(z_y)
        self.sigma2 = max(4.0, self.sigma2 * (1.0 - 0.5 * alpha))
    ...
    try:
        while True:
            ...
            if wall_centers_xy is not None:
                ...
                pose_kalman.predict(dx_p, dy_p, q_var_mm2=active_kalman_q)
                ...
                if do_match and match_tick_counter >= active_match_every:
                    ...
                    for _icp in range(active_icp_iters):
                        ...
                        if delta_iter < ICP_CONV_THRESH_MM:
                            break                    # Corrección de pose simplificada
                    ...
                    pose_kalman.update_lidar(_icp_x, _icp_y, r_var_mm2=r_var)

...
# ═══════════════════════════════════════════════════════
POSE_VIEW_FINE_DEG    =  1.0   # J/K ajustan SOLO la pose y conos ±1°
POSE_VIEW_COARSE_DEG  = 10.0   # Shift+J/K ajustan SOLO la pose y conos ±10°
...
def main():
    ...
    body_pose_mouse_offset_deg = saved_doc.get("body_pose_mouse_offset_deg", 0.0) if saved_doc else 0.0
    body_pose_adjust_mode = False
    frame_rotation_deg = saved_doc.get("frame_rotation_deg", 0.0) if saved_doc else 0.0
    pwr_idx = 0
    ...
    try:
        while True:
            ...
            for e in pygame.event.get():
                    ...
                    if e.key == pygame.K_q:
                        ...
                        if body_pose_adjust_mode:
                            try:
                                ...
                                pass
                            print("[BODY CAL] Modo ajuste ACTIVO — mueve el mouse horizontalmente para rotar el cuerpo")
                        else:
                            print(f"[BODY CAL] Guardado offset cuerpo: {body_pose_mouse_offset_deg:+.1f}°")
                ...
                if e.type == pygame.MOUSEMOTION and body_pose_adjust_mode:
                    dx = e.rel[0]
                    if dx != 0:
                        body_pose_mouse_offset_deg = (body_pose_mouse_offset_deg + dx * MOUSE_ROT_SENS) % 360.0
                        if body_pose_mouse_offset_deg > 180:
                            body_pose_mouse_offset_deg -= 360
            ...
            yaw_deg_motion = yaw_deg_real + body_pose_mouse_offset_deg
            ...
            if modo == MODE_SCAN:
                ...
                if fsm_state == FSM_RETURN_HOME:
                    ...
                    if return_home_inner_fsm == RH_APPROACH:
                        if dist_home < HOME_REVERSE_APPROACH_MM:
                            ...
                            if (not body_pose_adjust_mode) and home_path_idx < len(home_path):
                ...
                # ── Estados normales (IDLE/ALIGN/ADVANCE/BLOCKED) ─
                    ...
                    if (not body_pose_adjust_mode) and not in_stuck_recovery and not stuck_detected:
            ...
            route_clean_active = ((modo == MODE_RUTA and not body_pose_adjust_mode) and fsm_state in (FSM_ALIGN, FSM_ADVANCE, FSM_BLOCKED, FSM_WP_REACHED))
                ...
                if do_match and match_tick_counter >= active_match_every:
                    ...
                    if YAW_ICP_ENABLED and wall_centers_xy is not None:
                        ...
                        if abs(_yaw_iter - yaw_deg_real) > 0.05:
                            ...
                            yaw_deg_motion = yaw_deg_real + pose_view_yaw_offset_deg
                            yaw_deg_draw = yaw_deg_motion
                            yaw_deg = yaw_deg_motion
            ...
            if state_mgr.is_dirty(lidar_x_mm, lidar_y_mm, yaw_deg, grid_map):
                ...
                state_mgr.save({
                    ...
                    "body_pose_mouse_offset_deg": body_pose_mouse_offset_deg,
            ...
            if (now - last_render_time) >= INTERVALO_DIBUJO:
                ...
                # T + mouse rota el cuerpo representado del robot: conos, ROI y movimiento.

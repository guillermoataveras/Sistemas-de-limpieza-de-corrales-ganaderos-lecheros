...
# ==========================================
PATH_CHECK_EVERY_N   = 10     # verificar camino cada N ticks (~3 Hz)
COLOR_WP_SKIPPED     = (110, 110, 120)   # gris — waypoint saltado por pared
COLOR_PATH_BLOCKED   = (200,  50,  50)   # rojo oscuro — tramo bloqueado
...
def draw_panel(screen, font, small, state):
    ...
    card = draw_card(screen, cx, y, cw, 134, "RUTA", font, small)
    screen.blit(
    ...
    screen.blit(
        small.render("[Tab] entrar RUTA reinicia saltados", True, COLOR_SUBTEXT),
...
def main():
    ...
    # ── Fase E: grilla como filtro de seguridad ──────────
    skipped_wp_indices  = set()   # waypoints saltados por cruzar pared
    path_check_counter  = 0       # contador para verificación cada N ticks
                ...
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_F1:
                        ...
                        if modo != MODE_SCAN:
                            ...
                            scan_start_time = now
                            wall_centers_xy = None   # invalidar hasta nuevo scan
                            last_match_quality  = 0.0
                            last_match_corr_mm  = 0.0
                            modo = MODE_SCAN
                            pl, pr = 0.0, 0.0

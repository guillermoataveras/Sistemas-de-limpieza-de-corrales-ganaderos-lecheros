...
def _panel_section_ruta(screen, small, state, cx, cw, y):
    ...
    for s, c in [
        ...
        (f"Home: {'set ✓' if home_set else 'no set'}", COLOR_OK if home_set else COLOR_SUBTEXT),
...
def _panel_section_sistema(screen, small, state, cx, cw, y):
    ...
    for s, c in [
        (f"Arduino: {'OK' if arduino_ok else 'DESCONECTADO'}", COLOR_OK if arduino_ok else COLOR_WARN),
        (f"LiDAR:   {'OK' if lidar_ok else 'DESCONECTADO'}", COLOR_OK if lidar_ok else COLOR_WARN),
        (f"Encoders:{'OK' if enc_ok else 'N/A'}", COLOR_OK if enc_ok else COLOR_SUBTEXT),
        ...
        (f"Scan match: {match_q:.0f}%", COLOR_OK if match_q>50 else COLOR_WARN),
        (f"Descubierto: {disc_pct:.1f}%  Limpio: {clean_pct:.1f}%", COLOR_SUBTEXT),
        (f"Hits dyn: {dyn_t}  Cono: {frontal_c}", COLOR_ALERT if blocked else COLOR_SUBTEXT),
...
def follow_route_step(robot_x, robot_y, yaw_deg,
    ...
    if fsm_state == FSM_IDLE:
        ...
        max_corr = PWM_FOLLOWER_ADVANCE * 0.6
        if heading_pid is not None:
...
def main():
    ...
    return_home_inner_fsm = FSM_ALIGN   # sub-estado para RETURN_HOME
        ...
        while True:
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    # ── Panel section toggles ─────────────────────
                        ...
                        if modo == MODE_RUTA:
                            ...
                            skipped_wp_indices = set()
                            path_check_counter = 0
                            # Guardar posición de inicio de misión
                        ...
                        is_returning_home     = False
                        return_home_inner_fsm = FSM_ALIGN
                        backup_dist_done      = 0.0
                        wait_dyn_ticks        = 0
                        adaptive_paso_mm      = float(PASO_LIMPIEZA_MM)
                        cam_rear_cells_reset  = 0
            ...
            if modo == MODE_SCAN:
                ...
                scale_dyn  = obstacle_speed_scale(
                    nearest_dyn_dist, SLOW_ZONE_DYN_MM, STOP_ZONE_DYN_MM)
                scale_sta  = obstacle_speed_scale(
                    nearest_sta_dist, SLOW_ZONE_STA_MM, STOP_ZONE_STA_MM)
                ...
                if fsm_state == FSM_BACKUP:
                    if rear_blocked:
                        ...
                        # Retroceder usando encoders para medir distancia
                        # (usa los deltas ya leídos al inicio del tick)
                        if enc_available:
                ...
                # ── Estado RETURN_HOME ────────────────────────────
                    ...
                    # Navegación inteligente a home con A* + breadcrumbs.
                    # home_path se calcula una vez y se sigue waypoint por waypoint.
                    ...
                    if dist_home < WP_REACH_MM:
                        ...
                        # Construir ruta de retorno si aún no existe
                        if not home_path:
                            ...
                            home_path_idx = 0
                            print(f"[RETURN_HOME] Ruta calculada: {len(home_path)} waypoints")

                        # Avanzar al waypoint actual de home_path
                        if home_path_idx < len(home_path):
                            pl, pr, return_home_inner_fsm, new_hp_idx = follow_route_step(
                                lidar_x_mm, lidar_y_mm, yaw_deg,
                                home_path, home_path_idx,
                                return_home_inner_fsm,
                                frontal_blocked,
                                heading_pid=heading_pid,
                                dt=dt
                            )
                            # Avanzar al siguiente waypoint si se alcanzó
                            if return_home_inner_fsm == FSM_WP_REACHED:
                            ...
                            # Ruta agotada pero no llegamos a home: recalcular
                            home_path = []
            ...
            if wall_centers_xy is not None:
                # ── NIVEL 1: Encoders + ICP scan matching + Kalman ───
                # PREDICT con encoders (siempre que haya datos)
                if enc_available:
                    ...
                    if not slip_detected:
                        ...
                        dx_enc = lidar_x_mm - prev_x
                        dy_enc = lidar_y_mm - prev_y
                        pose_kalman.predict(dx_enc, dy_enc,
                                            q_var_mm2=KALMAN_Q_BASE_MM2)
                    else:
                        # Slip: no mover, pero aumentar incertidumbre
                ...
                if match_tick_counter >= MATCH_EVERY_N_TICKS:
                    ...
                    for _icp in range(ICP_ITERATIONS):
                        ...
                        delta_iter = math.hypot(_icp_x - _prev_x, _icp_y - _prev_y)
                        if delta_iter < ICP_CONV_THRESH_MM:
                            break

                    # UPDATE Kalman con medición del ICP
                    pose_kalman.update_lidar(_icp_x, _icp_y,
                                             match_quality=last_match_quality)
                    # La pose final es la del Kalman

...
def build_route_with_astar(pattern_waypoints, grid_map,
    ...
    for wp in pattern_waypoints:
        ...
        direct_ok = not grid_map.path_crosses_wall(prev[0], prev[1],
                                                    wp[0], wp[1])
        if direct_ok and not grid_map.has_wall_within(wp[0], wp[1],
                                                       clearance_mm):
            result.append(wp)
        else:
            # A* para encontrar ruta segura
            path = astar_path(grid_map, prev, wp, clearance_mm=clearance_mm)
            if path:
...
class LiDAR_LD20:
    ...
    def read_loop(self):
        ...
        while self.running:
            try:
                # ── Watchdog LiDAR ────────────────────────────────
                # Solo reconectar si hubo comunicación previa exitosa
                # (last_rx_time > 0) Y el silencio supera el umbral.
                # Evita falso positivo cuando el Arduino está quieto.
                if (self.last_rx_time > 0
                        and time.time() - self.last_rx_time > RECONNECT_TIMEOUT_S):
                    print(f"[LiDAR] Sin datos por {RECONNECT_TIMEOUT_S}s — reconectando...")
                    self.last_rx_time = 0.0   # reset para no re-disparar hasta nuevo pkt
                    self._try_reconnect_lidar()
                    buffer = b''
                    continue
    ...
    def _try_reconnect_lidar(self):
        """Intenta reabrir el puerto serial del LiDAR."""
        try:
            if self.serial:
            ...
            self.serial = serial.Serial(self.port, self.baudrate, timeout=1)
            self.last_rx_time = time.time()
            print(f"[LiDAR] Reconectado en {self.port}")
        except Exception as e:
            print(f"[LiDAR] Reintento fallido: {e}")
            time.sleep(RECONNECT_RETRY_S)
...
class ArduinoSerialController:
    ...
    def read_loop(self):
        while self.running:
            try:
                # ── Watchdog: detectar silencio y reconectar ──
                # Solo si hubo paquete previo (last_rx_time > 0); reset
                # para no re-disparar en rápida sucesión.
                if (self.last_rx_time > 0
                        and time.time() - self.last_rx_time > RECONNECT_TIMEOUT_S):
                    print(f"[Arduino] Sin datos por {RECONNECT_TIMEOUT_S}s — reconectando...")
                    self.last_rx_time = 0.0   # reset hasta recibir nuevo pkt
                    self._try_reconnect()
                    continue
    ...
    def _try_reconnect(self):
        """Intenta reabrir el puerto serial del Arduino."""
        try:
            if self.serial:
                try: self.serial.close()
                except Exception: pass
            ...
            self.serial = serial.Serial(self.port, self.baudrate, timeout=0.05)
            self.last_rx_time = time.time()
            print(f"[Arduino] Reconectado en {self.port}")
        except Exception as e:
            print(f"[Arduino] Reintento fallido: {e}")
            time.sleep(RECONNECT_RETRY_S)
    ...
    def draw(self, screen, area_rect):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                # ── Capa de suciedad: sobrescribe cleanable/cleaned ──
                if DIRT_GRID_ENABLED and self.cam_seen[r, c]:
                    d = self.dirt[r, c]
                ...
                # Capas normales si la cámara no ha visto la celda
                if self.cleaned[r, c]:
...
# RESET
# ==========================================
def do_replan(grid_map, paso_mm=None):
    """
    ...
    """
    wps, pat = replan_cleaning(grid_map, paso_mm=paso_mm)
    if not wps:
...
def draw_controls_dropdown(screen, small):
    ...
    items = [
        ...
        ("E","Exportar mapa PNG"), ("L","Exportar log CSV"), ("P","Reset mapa/yaw"),
...
def replan_cleaning(grid_map, paso_mm=None):
    """
    ...
    if DIRT_GRID_ENABLED:
        ...
        if dirt_wps:
            ...
            dirt_wps = filter_route_waypoints(dirt_wps, grid_map)
            dirt_wps = enforce_waypoint_spacing(dirt_wps, grid_map)
            if dirt_wps and grid_map.has_wall_map():
                dirt_wps = build_route_with_astar(dirt_wps, grid_map)
            if dirt_wps:
    ...
    wps = filter_route_waypoints(wps, grid_map)
    ...
    if wps and grid_map.has_wall_map():
        wps = build_route_with_astar(wps, grid_map)
                ...
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_e:
                        ...
                        if grid_map.has_cleanable_mask() and modo != MODE_SCAN:
                            wps, pat, ok = do_replan(grid_map, paso_mm=adaptive_paso_mm)
                            if ok:
                                ruta_waypoints     = wps
                                patron_actual      = pat
                                waypoint_idx       = 0

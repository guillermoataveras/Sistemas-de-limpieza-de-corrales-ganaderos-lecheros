...
BODY_ROT_FINE_DEG     =  1.0   # J/K ajustan cuerpo completo (sin nube) ±1° cuando T está activo
BODY_ROT_COARSE_DEG   =  5.0   # Shift+J/K ajustan cuerpo completo (sin nube) ±5° cuando T está activo
    ...
    try:
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    if e.key == pygame.K_q:
                        ...
                        if body_pose_adjust_mode:
                            print("[BODY CAL] Modo ajuste ACTIVO — usa J/K para rotar el cuerpo completo (sin nube)")
                        else:
                            print(f"[BODY CAL] Guardado offset cuerpo: {body_pose_mouse_offset_deg:+.1f}°")

                    elif e.key == pygame.K_j and body_pose_adjust_mode:
                        step = BODY_ROT_COARSE_DEG if _shift_held else BODY_ROT_FINE_DEG
                        body_pose_mouse_offset_deg = (body_pose_mouse_offset_deg - step) % 360.0
                        if body_pose_mouse_offset_deg > 180:
                            body_pose_mouse_offset_deg -= 360
                        print(f"[BODY CAL] Offset cuerpo: {body_pose_mouse_offset_deg:+.1f}°")
                ...
                yaw_deg_body = yaw_deg + body_pose_mouse_offset_deg
                grid_map.draw_robot(screen, map_rect, lidar_x_mm, lidar_y_mm, yaw_deg_body,

...
class LiDAR_LD20:
    ...
    def read_loop(self):
        ...
        while self.running:
            try:
                # ── Watchdog LiDAR ────────────────────────────────
                if (self.last_rx_time > 0 and
                        time.time() - self.last_rx_time > RECONNECT_TIMEOUT_S):
...
def main():
    ...
    slip_detected       = False
    gyro_prev_yaw       = 0.0   # para calcular delta de yaw por tick
        ...
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    # Toggle panel sections by clicking their headers
                    elif mx >= MAP_W:   # click is on the panel side
            ...
            if wall_centers_xy is not None:
                ...
                if match_tick_counter >= MATCH_EVERY_N_TICKS:
                    match_tick_counter = 0
                    # ICP iterativo: múltiples pasadas hasta convergencia
                    for _icp in range(ICP_ITERATIONS):
                        ...
                        if last_match_corr_mm < ICP_CONV_THRESH_MM:
                            break   # convergido

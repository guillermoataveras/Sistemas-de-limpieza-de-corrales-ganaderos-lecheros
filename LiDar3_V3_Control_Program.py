...
# ═══════════════════════════════════════════════════════
# AJUSTE ANGULAR DEL OFFSET DE YAW
# ═══════════════════════════════════════════════════════
# Teclas Q/E ajustan saved_yaw_offset (único offset de yaw tras Fase 2)
FRAME_ROT_FINE_DEG    =  1.0   # Q/E ajustan ±1°
FRAME_ROT_COARSE_DEG  = 10.0   # Shift+Q/E ajustan ±10°
...
def draw_toolbar(screen, font, small, state, dropdown_open=False):
    ...
    # Cloud rotation debug pill (herramienta de diagnóstico — tecla R)
    cloud_rot = state.get("cloud_rotation_debug_deg", 0.0)
    if abs(cloud_rot) > 0.05:
        cx = pill(f"☁ cloud {cloud_rot:+.1f}°", (255, 120, 200), (55, 20, 45), cx)
...
def draw_controls_dropdown(screen, small):
    ...
    items = [
        ...
        ("[ / ]",    "Umbral cobertura ± 5%"),
        # ── Calibración de yaw ─────────────
        ("Q / E",    "Ajustar saved_yaw_offset ± 1°"),
        ("Shift+Q/E","Ajustar saved_yaw_offset ± 10°"),
        ("G",        "DEBUG: rotar solo nube +1° (diagnóstico)"),
        ("Shift+G",  "DEBUG: rotar solo nube -1°"),
        ("Ctrl+G",   "DEBUG: reset rotación de nube"),
        # ── Zonas no-go ────────────────────
...
def main():
    ...
    # ── Offsets de yaw (Fase 2: consolidados) ─────────────
    # Antes había 3 offsets acumulativos (frame_rotation_deg,
    # body_pose_yaw_offset_deg, BODY_FRAME_OFFSET_DEG). Ahora solo existe
    # uno: saved_yaw_offset, que se inicializa más abajo según saved_doc o
    # FRAME_ROTATION_DEG. Para retrocompat con sesiones antiguas, cualquier
    # body_pose_yaw_offset_deg guardado se migra sumándolo a saved_yaw_offset.
    _legacy_body_offset = saved_doc.get("body_pose_yaw_offset_deg", 0.0) if saved_doc else 0.0
    body_pose_adjust_mode = False   # kept for legacy toggle key (T) — no-op ahora

    # ── Diagnóstico: rotación visual de la nube LiDAR ──────
    # Tecla R (+1°), Shift+R (-1°), Ctrl+R (reset). Afecta SOLO a la nube,
    # no a pose ni navegación. Sirve para medir visualmente cuánto hay que
    # rotar el offset de yaw para que la nube calce con el mapa real.
    # El valor leído en pantalla se debería sumar a saved_yaw_offset y
    # luego dejar cloud_rotation_debug_deg en 0 para operación normal.
    cloud_rotation_debug_deg = 0.0
    pwr_idx = 0
        ...
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    ...
                    if e.key == pygame.K_q:
                        # Q / Shift+Q: ajuste fino/grueso del offset único de yaw.
                        # Afecta a TODO (chasis, ROIs, conos, nube) por igual.
                        step = FRAME_ROT_COARSE_DEG if _shift_held else FRAME_ROT_FINE_DEG
                        saved_yaw_offset = (saved_yaw_offset - step + 540.0) % 360.0 - 180.0
                        print(f"[YAW] saved_yaw_offset: {saved_yaw_offset:+.1f}°")
                        ...
                        # T: legacy no-op — antes activaba modo "body adjust".
                        # Hoy todos los offsets están unificados en Q/E.
                        print("[INFO] Tecla T ya no tiene función: "
                              "usa Q/E para ajustar saved_yaw_offset.")

                    elif e.key == pygame.K_g:
                        # G: rotación de DEBUG de la nube (solo afecta la
                        # visualización de lidar_hits_global). Sirve para medir
                        # visualmente cuánto hay que corregir el offset de yaw.
                        # Shift+G → -1°, Ctrl+G → reset a 0°.
                        _ctrl_held = bool(_mods & pygame.KMOD_CTRL)
                        if _ctrl_held:
                            cloud_rotation_debug_deg = 0.0
                            print("[CLOUD DEBUG] Reset → 0.0°")
                        else:
                            step = -1.0 if _shift_held else 1.0
                            cloud_rotation_debug_deg = (
                                (cloud_rotation_debug_deg + step + 540.0) % 360.0 - 180.0
                            )
                            print(f"[CLOUD DEBUG] Rotación nube: "
                                  f"{cloud_rotation_debug_deg:+.1f}° "
                                  f"(sumar este valor a saved_yaw_offset si la "
                                  f"nube calza bien así)")
            ...
            # La nube usa yaw_deg + cloud_rotation_debug_deg para permitir
            # girarla aisladamente con la tecla G (diagnóstico visual).
            # En operación normal cloud_rotation_debug_deg = 0.
            yaw_for_cloud = yaw_deg + cloud_rotation_debug_deg
            lidar_hits_global = transform_scan_to_global(
                ang, dist, lidar_x_mm, lidar_y_mm, yaw_for_cloud
            )
            ...
            if (now - last_render_time) >= INTERVALO_DIBUJO:
                ...
                state = {
                    ...
                    "cloud_rotation_debug_deg": cloud_rotation_debug_deg,

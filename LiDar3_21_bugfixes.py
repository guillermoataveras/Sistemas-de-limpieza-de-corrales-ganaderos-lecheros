...
def _panel_section_pose(screen, small, state, cx, cw, y):
    ...
    screen.blit(small.render(f"Corr: {corr:.1f}mm", True, COLOR_SUBTEXT),(cx+10,y));y+=17
...
def replan_cleaning(grid_map, paso_mm=None):
    ...
    if not wps:
        ...
        # (corral muy pequeño o margen muy agresivo)
        ...
        while True:
            ...
            if modo == MODE_SCAN:
                ...
                # ── Estado RETURN_HOME ────────────────────────────
                    ...
                    # Navegar al home como un waypoint único
                    ...
                    if dist_home < WP_REACH_MM:
                        ...
                        # Reutilizar follower con home como waypoint temporal.
                        # return_home_inner_fsm rastrear ALIGN→ADVANCE sub-estado.
                        pl, pr, return_home_inner_fsm, _ = follow_route_step(
                            lidar_x_mm, lidar_y_mm, yaw_deg,
                            [target], 0,
            ...
            if wall_centers_xy is not None:
                # ── NIVEL 1: Encoders + ICP scan matching ──────────
                if enc_available and not slip_detected:
                    # Usar encoders solo si no hay slip
                ...
                if match_tick_counter >= MATCH_EVERY_N_TICKS:
                    match_tick_counter = 0
                    # ICP iterativo: converge cuando el desplazamiento entre
                    # iteraciones consecutivas es menor que ICP_CONV_THRESH_MM.
                    # last_pose_correction = distancia total corregida
                    # (posición inicial antes del ICP vs posición final).
                    ...
                    lidar_x_mm = _icp_x
                    lidar_y_mm = _icp_y
                    # FIX: medir contra la posición inicial, no contra sí misma

...
MATCH_TURN_SETTLE_TICKS     = 10       # ticks de espera tras parar giro
...
FSM_WAIT_DYNAMIC = "WAIT_DYN"      # esperando que despeje obstáculo dinámico
FSM_BACKUP       = "BACKUP"        # retrocediendo de forma segura
...
LIDAR_ONLY_MATCH_EVERY    = 2       # ICP cada 2 ticks (~15 Hz a 30 FPS)
LIDAR_ONLY_ICP_ITERS      = 6       # más iteraciones sin encoders
...
LIDAR_ONLY_FOLLOW_ALIGN   = 55      # follower PWM align (en vez de 65)
LIDAR_ONLY_FOLLOW_ADVANCE = 60      # follower PWM advance (en vez de 85)
...
# ═══════════════════════════════════════════════════════
# ROTACIÓN DE VISTA (solo visualización, datos internos = coords reales)
# ═══════════════════════════════════════════════════════
VIEW_ROT_FINE_DEG    =  1.0   # Q/E rotan ±1°
VIEW_ROT_COARSE_DEG  = 10.0   # Shift+Q/E rotan ±10°
...
def detect_wheel_slip(enc_delta_izq_deg, enc_delta_der_deg,
                       gyro_yaw_delta_deg):
    """
    Detecta deslizamiento de rueda comparando yaw calculado desde encoders
    con el yaw del giroscopio.

    Cuando una rueda patina, los encoders reportan más rotación de la real,
    lo que causa divergencia entre ambas fuentes.

    Retorna (slip_detected: bool, slip_magnitude_deg: float)
    """
    # Si el robot casi no se mueve, no hay referencia válida
    if abs(enc_delta_izq_deg) < 0.3 and abs(enc_delta_der_deg) < 0.3:
        return False, 0.0

    dist_izq = enc_delta_izq_deg * MM_PER_DEG_WHEEL
    dist_der = enc_delta_der_deg * MM_PER_DEG_WHEEL
    enc_yaw_deg = math.degrees((dist_der - dist_izq) / WHEEL_TRACK_MM)

    slip_mag = abs(enc_yaw_deg - gyro_yaw_delta_deg)
    return slip_mag > SLIP_THRESHOLD_DEG, slip_mag


def export_session_log(log_entries, base_path="."):
    """
    Exporta el log de sesión como CSV.
    Columnas: timestamp, x_mm, y_mm, yaw_deg, modo, fsm,
              coverage_pct, dirt_ratio, rear_dirt_ratio, cam_aux_pwm,
              slip_detected, speed_scale
    """
    import csv
    if not log_entries:
        return None
    fname = os.path.join(
        base_path,
        f"session_log_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
    try:
        ...
        print(f"[Log] Error al exportar: {e}")
        return None


# ==========================================
# ==========================================
# FILTRO DE KALMAN — FUSIÓN POSE (x, y)
# ==========================================

class PoseKalmanFilter:
    """
    Filtro de Kalman 2D para la posición (x, y) del robot.

    El yaw NO se fusiona — siempre viene del giroscopio. El filtro
    solo combina la predicción de los encoders con la corrección
    del scan matching (ICP).

    Modo "predict-only" entre ICPs: los encoders predicen la pose
    normalmente y se actualiza P (covarianza). Cuando llega una
    medición de ICP, se hace la corrección completa.

    Estado:  x = [px, py]           (posición en mm)
    Cov:     P = 2×2 (diagonal)     (incertidumbre en mm²)
    """

    def __init__(self, x0, y0, p_init=KALMAN_P_INIT_MM2):
        self.x = float(x0)
        self.y = float(y0)
        self.P = np.array([[p_init, 0.0],
                           [0.0, p_init]], dtype=np.float64)

    def predict(self, dx_mm, dy_mm, q_var_mm2=KALMAN_Q_BASE_MM2):
        """
        Paso de predicción con desplazamiento de encoders o PWM.

        dx_mm, dy_mm : desplazamiento integrado desde la última predicción
        q_var_mm2    : varianza del proceso. Usar KALMAN_Q_SLIP_MM2
                       cuando hay slip detectado para ignorar los encoders.

        La matriz F es identidad (modelo constante de posición);
        la nueva pose se suma como control input.
        """
        self.x += dx_mm
        self.y += dy_mm
        # P_new = F P Fᵀ + Q ; F = I → P_new = P + Q
        self.P[0, 0] += q_var_mm2
        self.P[1, 1] += q_var_mm2

    def update_lidar(self, z_x, z_y, match_quality=None,
                     r_var_mm2=None):
        """
        Corrección con medición del LiDAR (resultado de ICP).

        match_quality : 0–100. Si se provee, r_var se interpola:
            quality >= KALMAN_MATCH_Q_GOOD → R_GOOD (baja incertidumbre)
            quality = 0                    → R_BAD  (alta incertidumbre)
        r_var_mm2     : override directo (ignora match_quality)

        El LiDAR tiene prioridad alta por diseño: cuando la calidad
        del match es buena, su R es chica y domina sobre los encoders.
        """
        if r_var_mm2 is None:
            if match_quality is None:
                r_var_mm2 = KALMAN_R_GOOD_MM2
            else:
                q = clamp(match_quality, 0.0, KALMAN_MATCH_Q_GOOD)
                ratio = q / KALMAN_MATCH_Q_GOOD
                r_var_mm2 = (ratio * KALMAN_R_GOOD_MM2 +
                             (1.0 - ratio) * KALMAN_R_BAD_MM2)

        # Ganancia K = P / (P + R)
        k_x = self.P[0, 0] / (self.P[0, 0] + r_var_mm2)
        k_y = self.P[1, 1] / (self.P[1, 1] + r_var_mm2)
    ...
    def uncertainty_mm(self):
        """Retorna el radio de incertidumbre (desviación estándar) actual."""
...
class CattleTracker:
    """
    Sigue clusters de hits dinámicos como objetos persistentes ("vacas").
    
    Para cada tick:
      1. Agrupa hits dinámicos en clusters por proximidad (flood-fill)
      2. Asocia clusters con tracks existentes por nearest-neighbor
      3. Actualiza velocidad como EMA de (delta_pos / delta_t)
      4. Predice posición futura asumiendo velocidad constante
    
    Los tracks sin match durante CATTLE_TRACK_TTL_TICKS se descartan.
    """
    
    _next_id = 1
    
    def __init__(self):
        self.tracks = {}   # {id: {cx, cy, vx, vy, last_seen_t, hits_count, age_ticks}}
    
    def _cluster_hits(self, hits_xy):
        """Agrupa hits por proximidad usando flood-fill simple. O(N²) aceptable para N<200."""
        if len(hits_xy) == 0:
            return []
        ...
        visited = [False] * n
        clusters = []
        r2 = CATTLE_CLUSTER_RADIUS_MM * CATTLE_CLUSTER_RADIUS_MM
        ...
        for i in range(n):
            ...
            while queue:
                ...
                if visited[j]:
                    continue
                visited[j] = True
                cluster_pts.append(hits_xy[j])
                xj, yj = hits_xy[j]
                for k in range(n):
                    if not visited[k]:
                        xk, yk = hits_xy[k]
                        if (xk-xj)**2 + (yk-yj)**2 < r2:
                            queue.append(k)
            if len(cluster_pts) >= CATTLE_MIN_HITS:
                cx = sum(p[0] for p in cluster_pts) / len(cluster_pts)
                cy = sum(p[1] for p in cluster_pts) / len(cluster_pts)
                clusters.append({"cx": cx, "cy": cy, "n": len(cluster_pts)})
        return clusters
    
    def update(self, hits_xy, now, dt):
        """Principal: asocia clusters con tracks y actualiza velocidades."""
        clusters = self._cluster_hits(hits_xy)
        
        # ── Asociación por nearest-neighbor ──
        unmatched_clusters = list(range(len(clusters)))
        for tid, track in list(self.tracks.items()):
            best_ci = -1
            best_d2 = CATTLE_MATCH_RADIUS_MM ** 2
            for ci in unmatched_clusters:
                cl = clusters[ci]
                d2 = (cl["cx"] - track["cx"])**2 + (cl["cy"] - track["cy"])**2
                if d2 < best_d2:
                    best_d2 = d2
                    best_ci = ci
            ...
            if best_ci >= 0:
                cl = clusters[best_ci]
                # Actualizar velocidad con EMA
                if dt > 0.01:
                    new_vx = (cl["cx"] - track["cx"]) / dt
                    new_vy = (cl["cy"] - track["cy"]) / dt
                    # Filtrar ruido — velocidades imposibles
                    speed = math.hypot(new_vx, new_vy)
                    if speed < CATTLE_MAX_SPEED_MM_S:
                        a = CATTLE_VEL_EMA_ALPHA
                        track["vx"] = a * new_vx + (1-a) * track["vx"]
                        track["vy"] = a * new_vy + (1-a) * track["vy"]
                track["cx"] = cl["cx"]
                track["cy"] = cl["cy"]
                track["hits_count"] = cl["n"]
        ...
        # ── Crear tracks nuevos para clusters no asociados ──
        for ci in unmatched_clusters:
            cl = clusters[ci]
            tid = CattleTracker._next_id
            CattleTracker._next_id += 1
            self.tracks[tid] = {
                "cx": cl["cx"], "cy": cl["cy"],
                "vx": 0.0, "vy": 0.0,
                "hits_count": cl["n"],
        ...
        for tid in list(self.tracks.keys()):
            if self.tracks[tid]["age_ticks"] > CATTLE_TRACK_TTL_TICKS:
                del self.tracks[tid]
    
    def predict_at(self, horizon_s):
        """Predice la posición de cada track en horizon_s segundos."""
        result = []
        for tid, track in self.tracks.items():
            px = track["cx"] + track["vx"] * horizon_s
            py = track["cy"] + track["vy"] * horizon_s
    ...
    def path_conflict(self, robot_x, robot_y, target_x, target_y,
                       horizon_s=CATTLE_PREDICT_HORIZON_S,
                       buffer_mm=CATTLE_AVOID_RADIUS_MM):
        """
        Retorna la vaca cuya posición predicha intersecta el segmento
        robot→target dentro de buffer_mm. None si no hay conflicto.
        """
        preds = self.predict_at(horizon_s)
        best = None
        best_d = float('inf')
        
        for p in preds:
            # Distancia punto-segmento
            d = _point_to_segment_dist(p["px"], p["py"],
                                        robot_x, robot_y,
                                        target_x, target_y)
            if d < buffer_mm and d < best_d:
                best_d = d
                best = p
        return best
...
def _point_to_segment_dist(px, py, x1, y1, x2, y2):
    """Distancia mínima de (px,py) al segmento (x1,y1)-(x2,y2)."""
    dx, dy = x2 - x1, y2 - y1
    L2 = dx*dx + dy*dy
    if L2 < 1e-6:
...
class ArduinoSerialController:
    ...
    def read_loop(self):
        while self.running:
            try:
                ...
                if self.serial and self.serial.in_waiting:
                    ...
                    if line.startswith("DANG:"):
                        ...
                        try:
                            ...
                            if len(parts) == 4:
                                with self.lock:
                                    self.enc_acum_izq  = float(parts[0])
                                    self.enc_acum_der  = float(parts[1])
                                    self.enc_delta_izq = float(parts[2])
                                    self.enc_delta_der = float(parts[3])
                ...
                # Sin reconexión automática — evitar crash del hilo.
                time.sleep(0.05)
    ...
    def get_encoder_deltas(self):
        """Retorna (delta_izq_deg, delta_der_deg, available) y **resetea**
        los deltas internos atómicamente. Esto evita que el mismo delta
        sea consumido dos veces (por ejemplo: odometría + backup distance
        accumulation en el mismo tick) o que un delta "fantasma" siga
        activo cuando el Arduino deja de enviar por ruedas quietas.
        """
        with self.lock:
            di, dd, av = self.enc_delta_izq, self.enc_delta_der, self.enc_available
            # Reset atómico: el siguiente mensaje ENC: reescribirá los deltas
...
def draw_panel(screen, font, small, state, expanded_sections=None):
    ...
    FSM_COLORS = {
        ...
        FSM_SLOWDOWN_OBS: (255,160,40), FSM_WAIT_DYNAMIC: (255,110,50),
        FSM_BACKUP: (180,80,255), FSM_RETURN_HOME: (255,100,220),
    }
...
def _panel_section_pose(screen, small, state, cx, cw, y):
    ...
    k_un= state.get("kalman_uncertainty", 0.0)
...
def _panel_section_sistema(screen, small, state, cx, cw, y):
    ...
    slip_det   = state.get("slip_detected", False)
    slip_e     = state.get("slip_ema", 0.0)
    paso_mm    = state.get("adaptive_paso_mm", PASO_LIMPIEZA_MM)
    use_enc    = state.get("use_encoders", True)

    # ── Indicador de modo ─────────────────────────────
    mode_label = "MODO: HÍBRIDO (enc+lidar)" if use_enc else "MODO: LiDAR-ONLY"
    mode_color = (100, 180, 255)   if use_enc else (255, 160, 80)
    ...
    # Encoder/slip rows solo si se usan encoders
    if use_enc:
        rows.extend([
            (f"Encoders:{'OK' if enc_ok else 'N/A'}",
             COLOR_OK if enc_ok else COLOR_SUBTEXT),
            (f"Slip: {'⚠ DETECTADO' if slip_det else 'OK'}  EMA:{slip_e:.1f}°",
             COLOR_WARN if slip_det else COLOR_OK),
        ])
...
def draw_toolbar(screen, font, small, state, dropdown_open=False):
    ...
    if modo == MODE_RUTA:
        fc = {FSM_ALIGN:(255,220,60), FSM_ADVANCE:(80,220,255),
              ...
              FSM_WAIT_DYNAMIC:(255,110,50),
              FSM_BACKUP:(180,80,255),
    ...
    # Kalman emergency stop pill
    if state.get("kalman_alert", False):
        cx = pill("⚠ KALMAN STOP", (255,80,80), (90,15,15), cx)

    # Evasion pill
    if state.get("evasion_active", False):
        cx = pill("🐄 EVADIENDO", (255,180,100), (80,50,20), cx)
...
def follow_route_step(robot_x, robot_y, yaw_deg,
                      ...
                      dyn_alert, heading_pid=None, dt=0.033,
                      pwm_align=None, pwm_advance=None):
    """
    Ejecuta un paso de la máquina de estados del follower.

    heading_pid : instancia de HeadingPID (opcional). Si se provee, usa
                  control PID en lugar de proporcional puro. Se resetea
                  automáticamente al entrar en FSM_ALIGN.
    dt          : tiempo transcurrido en este tick (segundos).

    Retorna (pl, pr, new_state, new_wp_idx)
    """
    pl, pr = 0.0, 0.0
    ...
    if fsm_state == FSM_IDLE:
        ...
        # Resetear PID al iniciar cada alineación
        ...
        pwm = pwm_align if pwm_align is not None else PWM_FOLLOWER_ALIGN
        if heading_error > 0:
            pl, pr = -pwm, +pwm
        else:
            pl, pr = +pwm, -pwm
        ...
        # PID o proporcional puro según si se proveyó controlador
        adv_pwm = pwm_advance if pwm_advance is not None else PWM_FOLLOWER_ADVANCE
        max_corr = adv_pwm * 0.6
        if heading_pid is not None:
            raw_corr = heading_pid.compute(heading_error, dt)
        else:
            raw_corr = K_HEADING * heading_error
        correction = clamp(raw_corr, -max_corr, max_corr)
...
def main():
    ...
    # ── Selección de constantes activas según USE_ENCODERS ──
    if USE_ENCODERS:
        active_pwm_base         = PWM_BASE
        active_match_every      = MATCH_EVERY_N_TICKS
        active_icp_iters        = ICP_ITERATIONS
        active_kalman_q         = KALMAN_Q_BASE_MM2
        active_kalman_r_good    = KALMAN_R_GOOD_MM2
        active_kalman_r_bad     = KALMAN_R_BAD_MM2
        active_slow_dyn_mm      = SLOW_ZONE_DYN_MM
        active_slow_sta_mm      = SLOW_ZONE_STA_MM
        active_follow_align     = PWM_FOLLOWER_ALIGN
        active_follow_advance   = PWM_FOLLOWER_ADVANCE
    else:
        # Modo LiDAR-only: más lento, más scans, más iteraciones
        ...
        print("═" * 62)
        print("  MODO LiDAR-ONLY activado (USE_ENCODERS=False)")
        print(f"  PWM_BASE={active_pwm_base} (reducido desde {PWM_BASE})")
    ...
    pose_kalman         = PoseKalmanFilter(lidar_x_mm, lidar_y_mm)
    kalman_uncertainty  = KALMAN_P_INIT_MM2 ** 0.5
    ...
    backup_enc_ref_i  = 0.0
    backup_enc_ref_d  = 0.0
    ...
    # ── Cattle tracker (vacas con velocidad) ─────────────
    cattle_tracker        = CattleTracker()
    cattle_last_update_t  = 0.0
    ...
    # ── Recovery watchdogs ──────────────────────────────
    kalman_high_sigma_ticks = 0
    kalman_emergency_stop   = False
    map_corrupt_stuck_ticks = 0
    backup_mgr              = StateBackupManager(state_mgr.filepath)

    # ── Evasión de vacas ────────────────────────────────
    evasion_cooldown_ticks  = 0       # tick counter de cooldown
    evasion_active          = False   # true mientras seguimos un bypass WP
    evasion_original_wp_idx = -1      # índice del WP que retomaremos tras bypass
    ...
    try:
        while True:
            ...
            for e in pygame.event.get():
                ...
                if e.type == pygame.KEYDOWN:
                    # ── Rotación de vista (Q/E ±1°, Shift+Q/E ±10°) ──
                    _mods = pygame.key.get_mods()
                    _shift_held = bool(_mods & pygame.KMOD_SHIFT)

                    if e.key == pygame.K_q:
                        step = VIEW_ROT_COARSE_DEG if _shift_held else VIEW_ROT_FINE_DEG
                        grid_map.view_rotation_deg = (grid_map.view_rotation_deg - step) % 360.0
                        if grid_map.view_rotation_deg > 180: grid_map.view_rotation_deg -= 360
                        print(f"[VIEW] Rotación: {grid_map.view_rotation_deg:+.1f}°")

                    elif e.key == pygame.K_e and _shift_held:
                        # Shift+E = rotar vista clockwise (+10°)
                        grid_map.view_rotation_deg = (grid_map.view_rotation_deg
                                                       + VIEW_ROT_COARSE_DEG) % 360.0
                        if grid_map.view_rotation_deg > 180: grid_map.view_rotation_deg -= 360
                        print(f"[VIEW] Rotación: {grid_map.view_rotation_deg:+.1f}°")

                    elif e.key == pygame.K_e:
                        # E = rotar vista clockwise fino (+1°)
                        grid_map.view_rotation_deg = (grid_map.view_rotation_deg
                                                       + VIEW_ROT_FINE_DEG) % 360.0
                        if grid_map.view_rotation_deg > 180: grid_map.view_rotation_deg -= 360
                        print(f"[VIEW] Rotación: {grid_map.view_rotation_deg:+.1f}°")
                        ...
                        # Z = Recalibrar orientación: captura el yaw actual
                        # del giroscopio y lo vuelve 0° en el frame del mapa.
                        # Equivalente a poner FRAME_ROTATION_DEG = -gyro_raw
                        # pero aplicado en vivo.
                        raw_yaw = arduino.get_yaw()
                        saved_yaw_offset = -raw_yaw
                        # El frame cambió → breadcrumbs/pose history quedan
                        # en un marco distinto, limpiar
                        breadcrumbs.clear()
                        stuck_pose_history.clear()
                        print(f"[FRAME] Recalibrado: orientación física actual → 0° "
                              f"(gyro bruto era {raw_yaw:+.1f}°)")
                    ...
                    # ── Panel section toggles ─────────────────────
                        ...
                        log_tick_counter      = 0
                        heading_pid.reset()
                        # Reset Kalman y breadcrumbs
                        pose_kalman.reset(lidar_x_mm, lidar_y_mm)
                        kalman_uncertainty    = KALMAN_P_INIT_MM2 ** 0.5
                        breadcrumbs.clear()
                        home_path             = []
                        home_path_idx         = 0
                        ...
                        grid_map.view_rotation_deg = 0.0
                        # Reset cattle tracker + kalman watchdog
                        cattle_tracker.clear()
                        kalman_high_sigma_ticks = 0
                        kalman_emergency_stop   = False
                        # Reset evasión + corrupt-map detection
                        evasion_cooldown_ticks  = 0
                        evasion_active          = False
                        evasion_original_wp_idx = -1
                        map_corrupt_prev_x      = None
                        map_corrupt_prev_y      = None
            ...
            # FIX: leer encoders UNA sola vez por tick y compartir entre
            # pose pipeline y backup distance accumulation.
            # get_encoder_deltas() ahora consume los deltas (los pone en 0),
            # así que múltiples llamadas darían 0 a los consumidores posteriores.
            ...
            cam_ok = camera.is_ok()
            cam_roi_corners = compute_roi_corners(lidar_x_mm, lidar_y_mm, yaw_deg)
            ...
            # ── Cattle tracker: seguir vacas como objetos ─────────
            cattle_dt = now - cattle_last_update_t if cattle_last_update_t > 0 else dt
            cattle_last_update_t = now
            cattle_tracker.update(list(dynamic_hits_render), now, cattle_dt)
            ...
            # ── Evasión de vacas: trigger por evento ──────────────
            # Solo cuando: modo RUTA, FSM_ADVANCE, hay hits dinámicos,
            # cooldown expirado, no estamos ya en bypass, hay WP activo.
            if evasion_cooldown_ticks > 0:
                evasion_cooldown_ticks -= 1

            if (modo == MODE_RUTA
                    and fsm_state == FSM_ADVANCE
                    and dyn_count > 0
                    and evasion_cooldown_ticks == 0
                    and not evasion_active
                    and ruta_waypoints
                    and CATTLE_EVASION_ENABLED
                    and 0 <= waypoint_idx < len(ruta_waypoints)):
                target_wp = ruta_waypoints[waypoint_idx]
                cow = cattle_tracker.path_conflict(
                ...
                if cow is not None:
                    # Calcular bypass lateral
                    bypass = compute_lateral_bypass(
                        lidar_x_mm, lidar_y_mm,
                        target_wp[0], target_wp[1],
                        cow["cx"], cow["cy"],
                        grid_map=grid_map,
                    ...
                    if bypass is not None:
                        # Insertar bypass WP antes del target actual
                        ruta_waypoints.insert(waypoint_idx, bypass)
                        evasion_active          = True
                        evasion_original_wp_idx = waypoint_idx + 1
                        evasion_cooldown_ticks  = CATTLE_EVADE_COOLDOWN_TICKS
                        fsm_state = FSM_ALIGN   # re-alinear al bypass
                        heading_pid.reset()
                        print(f"[EVADE] Vaca ID={cow['id']} en path → bypass "
                              f"lateral ({bypass[0]:.0f},{bypass[1]:.0f})")

            # Limpiar flag evasion_active cuando cerca del bypass WP
            if evasion_active:
                if waypoint_idx >= evasion_original_wp_idx:
                    # Ya pasamos el bypass, retomamos curso original
                    evasion_active = False
                    print("[EVADE] Bypass completado")
            ...
            if modo == MODE_SCAN:
                ...
                _, frontal_sta_hits = check_frontal_obstacle(
                    static_hits_render,  lidar_x_mm, lidar_y_mm, yaw_deg)
                ...
                # ── Estado BACKUP ─────────────────────────────────
                if fsm_state == FSM_BACKUP:
                    if rear_blocked:
                        # Atrás también bloqueado → quedarse quieto
                        pl, pr = 0.0, 0.0
                    else:
                        # Medir distancia recorrida en reversa:
                        # - Con encoders: usar los deltas leídos al inicio del tick
                        # - Sin encoders (LiDAR-only): usar desplazamiento
                        #   acumulado de la pose entre ticks
                        if USE_ENCODERS and enc_available:
                            d_step = abs((enc_delta_izq + enc_delta_der)
                                         * 0.5 * MM_PER_DEG_WHEEL)
                            backup_dist_done += d_step
                        else:
                            # LiDAR-only: diferencia entre pose actual y
                            # pose del tick anterior (capturada via backup_prev_x/y)
                            if backup_prev_x is not None:
                                d_step = math.hypot(lidar_x_mm - backup_prev_x,
                                                     lidar_y_mm - backup_prev_y)
                                backup_dist_done += d_step
                            backup_prev_x = lidar_x_mm
                            backup_prev_y = lidar_y_mm

                        if backup_dist_done >= BACKUP_DIST_MM:
                            # Distancia alcanzada → reintentar ruta
                            fsm_state        = FSM_ALIGN
                            backup_dist_done = 0.0
                            backup_prev_x    = None
                            backup_prev_y    = None
                            ...
                            # Retroceder (negativo = atrás)
                            pl = -float(BACKUP_PWM)
                            pr = -float(BACKUP_PWM)

                # ── Estado WAIT_DYNAMIC ───────────────────────────
                elif fsm_state == FSM_WAIT_DYNAMIC:
                    pl, pr = 0.0, 0.0
                    wait_dyn_ticks += 1
                    if wait_dyn_ticks >= WAIT_DYN_TICKS:
                        wait_dyn_ticks = 0
                        if not rear_blocked:
                            fsm_state        = FSM_BACKUP
                            backup_dist_done = 0.0
                        # Si atrás bloqueado: seguir esperando

                # ── Estado RETURN_HOME ────────────────────────────
                elif fsm_state == FSM_RETURN_HOME:
                    ...
                    # ── FASE 2: TURN_BACK (giro 180°) ──────────────────
                        ...
                        heading_error = _normalize_angle(reverse_heading - yaw_deg)
                ...
                # ── Estados normales (IDLE/ALIGN/ADVANCE/BLOCKED) ─
                    ...
                    # Aplicar desaceleración proporcional en ADVANCE
                    if fsm_state == FSM_ADVANCE and speed_scale < 1.0:
                        if speed_scale == 0.0:
                            # Zona crítica: parar y decidir qué hacer
                            pl, pr = 0.0, 0.0
                            is_dyn_threat = (nearest_dyn_dist < nearest_sta_dist)
                            if is_dyn_threat:
                                fsm_state = FSM_WAIT_DYNAMIC
                                wait_dyn_ticks = 0
                            else:
                                # Estático: intentar backup directamente
                                if not rear_blocked:
                                    fsm_state        = FSM_BACKUP
                                    backup_dist_done = 0.0
                                else:
                                    fsm_state = FSM_BLOCKED
                        else:
                            # Zona de desaceleración: frenar proporcionalmente
                            pl *= speed_scale
                            pr *= speed_scale
                            # Mostrar SLOWDOWN en panel (sin cambiar fsm real)
                            # fsm_state permanece ADVANCE internamente
            ...
            # aux_pwm final: visión adaptativa cuando clean_sys activo,
            # calculado ya arriba en el bloque de cámara.
            aux_pwm = cam_aux_pwm
            ...
            # ── Slip detection (compara encoders vs giroscopio) ──────
            gyro_yaw_delta = yaw_deg - gyro_prev_yaw
            gyro_prev_yaw  = yaw_deg
            if USE_ENCODERS and enc_available:
                slip_detected, slip_mag = detect_wheel_slip(
                    enc_delta_izq, enc_delta_der, gyro_yaw_delta)
                slip_ema = ((1.0 - SLIP_EMA_ALPHA) * slip_ema
                            + SLIP_EMA_ALPHA * (slip_mag if slip_detected else 0.0))
            else:
                # Sin encoders no se puede detectar patinaje
                slip_detected = False
                slip_ema      = 0.0

            if wall_centers_xy is not None:
                # ── NIVEL 1: Scan matching + Kalman ────────────────
                # PREDICT: encoders (si habilitados) o PWM integration
                prev_x, prev_y = lidar_x_mm, lidar_y_mm

                if USE_ENCODERS and enc_available:
                    # Predicción con encoders
                    if not slip_detected:
                        ...
                        pose_kalman.predict(dx_p, dy_p,
                                             q_var_mm2=active_kalman_q)
                    else:
                        # Slip detectado: aumentar incertidumbre
                        pose_kalman.predict(0.0, 0.0,
                                             q_var_mm2=KALMAN_Q_SLIP_MM2)
                else:
                    # LiDAR-only: predicción por integración de PWM
                    # Usa los comandos que se enviaron el tick anterior
                    # (last_pl_cmd, last_pr_cmd) — deshacer SIGNO para obtener
                    # la velocidad física real.
                ...
                # UPDATE: scan matching con ICP iterativo
                # ── Detectar si el robot está girando ──
                pwm_diff = abs(last_pl_cmd - last_pr_cmd)
                is_turning = (pwm_diff >= MATCH_TURN_PWM_DIFF_THR)

                if is_turning:
                    # En giro: pausar matching y reiniciar counter de settle
                    turn_settle_counter = MATCH_TURN_SETTLE_TICKS
                elif turn_settle_counter > 0:
                    # Recién terminó giro: esperar settle
                    turn_settle_counter -= 1

                was_turning_prev = is_turning
                ...
                if do_match and match_tick_counter >= active_match_every:
                    ...
                    # ── PASO 1: YAW ICP (corrección angular) ──
                    # Iteramos hasta convergencia del yaw antes de traslación.
                    # Esto corrige la orientación del scan respecto al mapa.
                    if YAW_ICP_ENABLED and wall_centers_xy is not None:
                        ...
                        for _ki in range(active_icp_iters):
                            ...
                            (_yaw_iter,
                             last_yaw_match_q,
                             last_yaw_correction) = yaw_icp_step(
                                lidar_x_mm, lidar_y_mm, _yaw_iter,
                                lidar_hits_local, wall_centers_xy
                            )
                            if abs(_yaw_iter - _prev_yaw) < YAW_ICP_CONV_THRESH_DEG:
                                break
                        # Aplicar el yaw corregido y regenerar hits globales
                        if abs(_yaw_iter - yaw_deg) > 0.05:
                            # Aplicar el delta al offset del giroscopio
                            delta_yaw = _yaw_iter - yaw_deg
                            saved_yaw_offset += delta_yaw
                            yaw_deg = _yaw_iter
                            # Regenerar hits globales con el nuevo yaw
                            lidar_hits_global = transform_scan_to_global(
                                ang, dist, lidar_x_mm, lidar_y_mm, yaw_deg
                            )

                    # ── PASO 2: TRASLACIÓN ICP (x, y) ──
                    ...
                    for _icp in range(active_icp_iters):
                        ...
                        delta_iter = math.hypot(_icp_x - _prev_x,
                                                 _icp_y - _prev_y)
                        if delta_iter < ICP_CONV_THRESH_MM:
                            break

                    # Kalman update con R adaptativo por match_quality
                    if USE_ENCODERS:
                        pose_kalman.update_lidar(_icp_x, _icp_y,
                                                  match_quality=last_match_quality)
                    else:
                        # R interpolado entre LIDAR_ONLY_R_GOOD/R_BAD
                        q = clamp(last_match_quality, 0.0, KALMAN_MATCH_Q_GOOD)
                        ratio = q / KALMAN_MATCH_Q_GOOD
                        r_var = (ratio * active_kalman_r_good +
                                 (1.0 - ratio) * active_kalman_r_bad)
                        pose_kalman.update_lidar(_icp_x, _icp_y,
                                                  r_var_mm2=r_var)

                    lidar_x_mm = pose_kalman.x
                    lidar_y_mm = pose_kalman.y
                    last_pose_correction = math.hypot(
                        lidar_x_mm - _icp_start_x,
                        lidar_y_mm - _icp_start_y
                    )
                else:
                    # Entre ICPs, la pose viene del Kalman (predict-only)
                    lidar_x_mm = pose_kalman.x
                    lidar_y_mm = pose_kalman.y

                kalman_uncertainty = pose_kalman.uncertainty_mm()

                # ── Watchdog Kalman: σ sostenido → reset + STOP ──
                if kalman_uncertainty > KALMAN_RESET_THRESHOLD_MM:
                    kalman_high_sigma_ticks += 1
                    if (kalman_high_sigma_ticks >= KALMAN_RESET_WINDOW_TICKS
                            and not kalman_emergency_stop):
                        print(f"[RECOVERY] Kalman σ={kalman_uncertainty:.1f}mm "
                              f"sostenido >{KALMAN_RESET_WINDOW_TICKS} ticks — "
                              f"RESET + STOP EMERGENCIA")
                        # Hard reset del Kalman a la última pose ICP buena
                        pose_kalman.reset(lidar_x_mm, lidar_y_mm)
                        kalman_uncertainty      = KALMAN_P_INIT_MM2 ** 0.5
                        kalman_emergency_stop   = True
                        kalman_high_sigma_ticks = 0
                        modo       = MODE_MANUAL
                        fsm_state  = FSM_IDLE
                        pl, pr     = 0.0, 0.0
                    ...
                    kalman_high_sigma_ticks = max(0, kalman_high_sigma_ticks - 1)

            elif USE_ENCODERS and enc_available:
                # ── NIVEL 2a: Encoders solos (sin mapa aún) ────────
                lidar_x_mm, lidar_y_mm = odometry_encoder_step(
                    lidar_x_mm, lidar_y_mm, yaw_deg,
                    enc_delta_izq, enc_delta_der
                )
                ...
                # ── Sin encoders ni mapa ni PWM: pose congelada ─────
                last_match_quality   = 0.0
                last_match_corr_mm   = 0.0
                last_pose_correction = 0.0
            ...
            if (now - last_render_time) >= INTERVALO_DIBUJO:
                ...
                if USE_ENCODERS:
                    _pose_src = (
                        "MATCH+GYRO" if (wall_centers_xy is not None and slip_detected)
                        else "MATCH+ENC" if (wall_centers_xy is not None and enc_available)
                        else "GYRO"     if slip_detected
                        else "ENCODERS" if enc_available
                        else "NO SENSORS"
                    )
                else:
                    _pose_src = (
                ...
                # ── Dibujar tracks de vacas ─────────────────
                for trk in cattle_tracker.tracks.values():
                    sx, sy = grid_map.world_to_screen(
                        map_rect, trk["cx"], trk["cy"])
                    r_px, _ = grid_map.world_to_screen(
                        map_rect, trk["cx"] + CATTLE_AVOID_RADIUS_MM, trk["cy"])
                    radius = max(4, int(abs(r_px - sx)))
                    # Círculo del track
                    ...
                    if speed > 50:   # > 50mm/s
                        tx = trk["cx"] + trk["vx"] * CATTLE_PREDICT_HORIZON_S
                        ty = trk["cy"] + trk["vy"] * CATTLE_PREDICT_HORIZON_S
                        psx, psy = grid_map.world_to_screen(map_rect, tx, ty)
                ...
                grid_map.draw_robot(screen, map_rect, lidar_x_mm, lidar_y_mm, yaw_deg,
                                    ...
                                    uncertainty_mm=kalman_uncertainty,
                ...
                state = {
                    ...
                    "view_rotation_deg":  grid_map.view_rotation_deg,
                    ...
                    "cattle_count":       len(cattle_tracker.tracks),
                    "kalman_alert":       kalman_emergency_stop,

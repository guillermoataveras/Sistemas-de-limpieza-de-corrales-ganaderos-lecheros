...
//      Formato: CMD:motor_izq,motor_der,aux_pwm,aux_enable,relay_open
...
// Formato: ACK:izq,der,aux,comp,relay_open,bomba_EL,cep_EL
...
struct CommandState {
  ...
  bool    auxEnable  = false;  // activa o no los BTS7960 auxiliares
...
void driveAuxiliaries(uint8_t pwmValue, bool enableAll) {
  // Enables siempre HIGH para que los drivers BTS7960 operen
...
//     → bomba/cepillo PWM=200 activados
//     → relé cerrado (compresor ON)
//
// Retrocompatibilidad: si llega un CMD con solo 4 campos (sin
// relay_open), se asume relay_open=0 (compresor normal).
...
bool parseCommandLine(const String &line, CommandState &out) {
  ...
  // Buscar las comas (hasta 4 separadores = 5 campos)
  ...
  int p4 = payload.indexOf(',', p3+1);   // puede o no existir
  ...
  if (p4 < 0) {
    // Formato antiguo (4 campos): el último campo es aux_enable
    out.auxEnable = (payload.substring(p3 + 1).toInt() != 0);
    out.relayOpen = false;
  } else {
    // Formato nuevo (5 campos): aux_enable, relay_open
    out.auxEnable = (payload.substring(p3 + 1, p4).toInt() != 0);
